...
{ 0x81E810,{ 0xC6, 0x05, 0x30, 0x09, 0x82, 0x00, 0x00, 0xE8 } }, //FunnyZhyper
{ 0x4B17F0,{ 0xE8, 0x70, 0x02, 0x00, 0x00, 0xE9, 0x80, 0xFE } },
{ 0x67A977,{ 0xE8, 0x71, 0xDA, 0x00, 0x00, 0xE9, 0x79, 0xFE } },
{ 0x950000,{ 0x56, 0x50, 0x53, 0xE8, 0x01, 0x00, 0x00, 0x00 } }  //| Fortis Fortuna Adiuvat |
};

void TEMUKOACSEngine::Dump_Scan()
{
	...
	if (hProc != INVALID_HANDLE_VALUE)
	{
		...
		do
		{
			for (int b = 0; b < 30 * SECOND; b++)
			{
				HANDLE hProcess = OpenProcess(PROCESS_ALL_ACCESS, false, b);
				if (hProcess != NULL)
				{
					for (int i = 0; i < (sizeof(mDumps) / sizeof(MDump)); i++)
					{
						...
						if (ReadProcessMemory(hProcess, (LPCVOID)mDumps[i].dwAddress, (LPVOID)pBytes, 8, &pBytesRead))
						{
							if (pBytesRead == 8)
							{
								...
								{
									g_bRunning = DUMP_SCAN;
									TerminateProcess(hProcess, NULL);
									OnLog(true, PC_DUMP_DETECTED, b);
								}
							}
						}
