...
public:
	...
	virtual bool Listen(uint16 sPort, uint16 sTotalSessions, bool isBlocking = true);
	virtual bool Listen(std::string sIPAddress, uint16 sPort, uint16 sTotalSessions, bool isBlocking = true);
...
template <class T>
bool KOSocketMgr<T>::Listen(uint16 sPort, uint16 sTotalSessions, bool isBlocking)
{
	return Listen("0.0.0.0", sPort, sTotalSessions, isBlocking);
}

template <class T>
bool KOSocketMgr<T>::Listen(std::string sIPAddress, uint16 sPort, uint16 sTotalSessions, bool isBlocking)
{
	if (m_server != nullptr)
		return false;
	...
	m_server = new ListenSocket<T>(this, sIPAddress.c_str(), sPort, isBlocking);
	if (!m_server->IsOpen())
		return false;
