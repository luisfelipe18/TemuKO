...
Socket::Socket(SOCKET fd, uint32 sendbuffersize, uint32 recvbuffersize, bool s_bKeepAlive) : m_fd(fd), m_connected(false), m_bloggedIn(false), m_deleted(false), m_socketMgr(nullptr), m_bOfflineSystemSocketType(false), m_bOfflineSystemTimeType(false)
{
	m_bKeepAlive = s_bKeepAlive;
...
bool Socket::Connect(const char* Address, uint32 Port, bool isBlocking)
{
...
void Socket::Disconnect()
{
	...
	{
		if (isOfflineSystemTime() <= 0)
		{
			m_bOfflineSystemTimeType = true;
			m_bOfflineSystemTime = (uint32_t(UNIXTIME) + 36000); // 36000 = 10 saat   Offline Merchant Süresi
		}
		return;
	}
